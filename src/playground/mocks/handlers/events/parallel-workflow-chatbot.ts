...
export const parallelWorkflowChatbotHandler = http.post(
  "/api/chat-parallel",
  async () => {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        controller.enqueue(
          encoder.encode(createNodeEvents.started(planningNode))
        );
        await sleep(500);
        controller.enqueue(
          encoder.encode(createNodeEvents.finished(planningNode))
        );
        await sleep(300);
